...
class ColorRegistry {
  ...
  registerColor (id, defaults, description, needsTransparency, deprecationMessage) {
    if (defaults.light === null || defaults.light === undefined) {
      // ignore
    } else if (typeof defaults.light === 'function') {
      this.defaultThemeLight[id] = defaults.light()
    } else if (typeof defaults.light === 'object') {
      this.defaultThemeLight[id] = defaults.light.hex
    } else if (typeof defaults.light === 'string') {
      if (defaults.light.startsWith('#')) {
        this.defaultThemeLight[id] = defaults.light
      } else {
        this.defaultThemeLight[id] = this.defaultThemeLight[defaults.light]
      }
    } else {
      this.defaultThemeLight[id] = defaults.light
    }

    if (defaults.dark === null || defaults.dark === undefined) {
      // ignore
    } else if (typeof defaults.dark === 'function') {
      this.defaultThemeDark[id] = defaults.dark()
    } else if (typeof defaults.dark === 'object') {
      this.defaultThemeDark[id] = defaults.dark.hex
    } else if (typeof defaults.dark === 'string') {
      if (defaults.dark.startsWith('#')) {
        this.defaultThemeDark[id] = defaults.dark
      } else {
        this.defaultThemeDark[id] = this.defaultThemeDark[defaults.dark]
      }
    } else {
      this.defaultThemeDark[id] = defaults.dark
    }

    if (defaults.contrast === null || defaults.contrast === undefined) {
      // ignore
    } else if (typeof defaults.contrast === 'function') {
      this.defaultThemeContrast[id] = defaults.contrast
    } else if (typeof defaults.contrast === 'object') {
      this.defaultThemeContrast[id] = defaults.contrast.hex
    } else if (typeof defaults.contrast === 'string') {
      if (defaults.light.startsWith('#')) {
        this.defaultThemeContrast[id] = defaults.contrast
      } else {
        this.defaultThemeContrast[id] = this.defaultThemeContrast[defaults.contrast]
      }
    } else {
      this.defaultThemeContrast[id] = defaults.contrast
    }
