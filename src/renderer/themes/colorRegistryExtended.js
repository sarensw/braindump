// The following registry is the extended registry. The colors here might
// depend on other colors that are based on the default theme + custom theme.
//
...
export function registerExtendedColors (type, intermediaryTheme) {
  const extendedTheme = {
    ...intermediaryTheme
  }

  const registerColor = (id, defaults, nls) => {
    if (defaults[type] === null) {
      return extendedTheme[id]
    } else if (typeof defaults[type] === 'function') {
      extendedTheme[id] = defaults[type]()
    } else if (typeof defaults[type] === 'object') {
      extendedTheme[id] = defaults[type].hex
    } else if (typeof defaults[type] === 'string') {
      if (defaults[type].startsWith('#')) {
        extendedTheme[id] = defaults[type]
      } else {
        extendedTheme[id] = extendedTheme[defaults[type]]
      }
    } else {
      extendedTheme[id] = defaults[type]
    }
    // extndedTheme[id] = defaults[type]
    return extendedTheme[id]
  }

  // Dependencies... see info below
  const editorBackground = intermediaryTheme['editor.background']

  // the following code is copied over from https://github.com/microsoft/vscode/blob/c980ea2307905fd1efeee38915bf1e5ccdff1754/src/vs/workbench/common/theme.ts
  // don't change!
  ...
  const STATUS_BAR_PROMINENT_ITEM_BACKGROUND = registerColor('statusBarItem.prominentBackground', {
  ...
  const STATUS_BAR_ERROR_ITEM_FOREGROUND = registerColor('statusBarItem.errorForeground', {
    dark: Color.white,
    light: Color.white,
    hc: Color.white
  ...
  const STATUS_BAR_HOST_NAME_BACKGROUND = registerColor('statusBarItem.remoteBackground', {
  ...
  const STATUS_BAR_HOST_NAME_FOREGROUND = registerColor('statusBarItem.remoteForeground', {
